  ...
  for (;;) {
    ...
    if (
      (optional &&
        ...
        this.tok.type !== tt.bracketL &&
        this.tok.type !== tt.backQuote) ||
      this.eat(tt.dot)
    ) {
      let node = this.startNodeAt(start);
      node.object = base;
...
lp.parseExprAtom = function () {
  ...
  switch (this.tok.type) {
    ...
    case tt.string:
      ...
      node.raw = this.input.slice(this.tok.start, this.tok.end);
      if (
        this.tok.type === tt.num &&
        node.raw.charCodeAt(node.raw.length - 1) === 110
      )
        node.bigint =
          node.value != null
            ? node.value.toString()
...
lp.toAssignable = function (node, binding) {
  if (
    ...
    for (let prop of node.properties) this.toAssignable(prop, binding);
...
lp.toAssignableList = function (exprList, binding) {
  for (let expr of exprList) this.toAssignable(expr, binding);
  return exprList;
};
