...
base.SwitchCase = (node, st, c) => {
  ...
  for (let cons of node.consequent) c(cons, st, "Statement");
};
...
base.VariableDeclaration = (node, st, c) => {
  for (let decl of node.declarations) c(decl, st);
};
...
base.NewExpression = base.CallExpression = (node, st, c) => {
  ...
  if (node.arguments) for (let arg of node.arguments) c(arg, st, "Expression");
};
...
base.ExportNamedDeclaration = base.ExportDefaultDeclaration = (node, st, c) => {
  ...
  if (node.attributes) for (let attr of node.attributes) c(attr, st);
};
...
base.ImportDeclaration = (node, st, c) => {
  for (let spec of node.specifiers) c(spec, st);
  c(node.source, st, "Expression");
  for (let attr of node.attributes) c(attr, st);
};
...
base.ClassBody = (node, st, c) => {
  for (let elt of node.body) c(elt, st);
};
