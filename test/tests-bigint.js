...
const digits = [
  ...
  {
    d: "-0xbf2ed51ff75d380fd3be813ec6185780",
    ast: (start) => ({
      start: start,
      ...
      operator: "-",
      prefix: true,
      argument: newBigIntLiteral(
        start + 1,
        "0xbf2ed51ff75d380fd3be813ec6185780",
        "254125715536285641815112686497309415296"
      ),
    }),
  },
  {
    d: "02",
    error: (start) => `Identifier directly after number (1:${start + 2})`,
  },
  {
    d: "2e2",
    error: (start) => `Identifier directly after number (1:${start + 3})`,
  },
  {
    d: "2.4",
    error: (start) => `Identifier directly after number (1:${start + 3})`,
...
statements.forEach((statement) => {
  ...
  digits.forEach((d) => {
    (d.error ? testFail : test)(
      ...
      d.error
        ? d.error(start)
        : { type: "Program", body: [statement.ast(d.ast(start))] },
      { ecmaVersion: 11 }
    );
  });
});
