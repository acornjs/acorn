...
lp.parseClassElement = function () {
  ...
  if (!keyName && ecmaVersion >= 8 && this.eatContextual("async")) {
    if (
      (this.isClassElementNameStart() || this.toks.type === tt.star) &&
...
lp.parseWithClause = function () {
  ...
  while (
    ...
    attr.key =
      this.tok.type === tt.string ? this.parseExprAtom() : this.parseIdent();
    if (this.eat(tt.colon)) {
      if (this.tok.type === tt.string) attr.value = this.parseExprAtom();
